...
function QueueItem({
  ...
  return (
    <div
      ...
      onKeyDown={(e) => {
        if ((e.key === 'Enter' || e.key === ' ') && job.status === 'pending') {
          onLoad(job);
        }
      }}
    ...
    >
      ...
      {job.status === 'running' && progress[job.id] !== undefined && (
        <div className="tw-job-progress">
          <div className="tw-job-progress-bar" style={{ width: `${String(progress[job.id])}%` }} />
        </div>
      )}
    </div>
  );
...
function SubmitButtons({
  ...
  if (editingJobId === null) {
    return (
      <button className="tw-btn tw-btn--primary" disabled={!canSubmit} onClick={onAddToQueue}>
        加入队列
      </button>
    );
  }
  return (
    <>
      <button className="tw-btn tw-btn--secondary" onClick={onCancelEdit}>
        取消编辑
      </button>
      <button className="tw-btn tw-btn--primary" disabled={!canSubmit} onClick={onUpdateJob}>
        更新任务
      </button>
    </>
  );
  ...
  return (
    <div className="tw-output">
      ...
      <div className="tw-output-row">
        ...
        <button className="tw-btn tw-btn--secondary" onClick={onPickOutputDir}>
          选择文件夹
        </button>
      </div>
      <div className="tw-output-row">
  ...
  return (
    <div className="tw-queue">
      <div className="tw-queue-header">
        ...
        <button className="tw-btn tw-btn--secondary tw-btn--sm" onClick={onNewDraft}>
          + 新建
        </button>
      </div>
